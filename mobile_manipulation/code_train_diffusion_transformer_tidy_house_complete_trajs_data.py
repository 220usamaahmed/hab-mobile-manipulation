...
def train_diffusion_model(upload_directory,save_directory,num_prev_obs=5, num_predicted_actions=20):
   ...
   # model.load_state_dict(torch.load(os.path.join(save_directory, 'model_16850.pt'))  )
    model.to(device)
        ...
        #    print("Processing file: ", file_name , " with number of episodes == " , number_of_episodes_in_file)
            for episode_key in data.keys():
                ...
                for batch in range(number_of_batches):
                    ...
                    visual_obs_batch=torch.tensor(visual_obs_data[start_idx:end_idx]).to(device)
                    non_visual_obs_batch=torch.tensor(non_visual_obs_data[start_idx:end_idx]).to(device)
                    action_batch=torch.tensor(action_data[start_idx:end_idx]).to(device)
