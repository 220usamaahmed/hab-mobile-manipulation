    ...
    for i_ep in range(num_episodes):
        ...
        while True:
            ...
            current_step_obs_transformer["relative_resting_position"] = (
                torch.tensor(local_ee_pos_relative_to_base - resting_pos)
            )
            ...
            current_step_obs_transformer["obj_start_gps_compass"] = (
                torch.tensor(relative_pick_pos_base_polar)
            )
            current_step_obs_transformer["obj_goal_gps_compass"] = (
                torch.tensor(relative_place_pos_base_polar)
            )
            ...
            if receptacle_number == 2:
                current_step_obs_transformer["all_predicates"][0:5] = (
            ...
            if (
                ...
                and current_task_name != "reset_arm"
            ):
                ...
                current_step_action_transformer[0:7] = torch.tensor(
                    action_to_save[0][2:9]
                )
                current_step_action_transformer[9] = torch.tensor(
                    action_to_save[0][9]
                )
                current_step_action_transformer[7:9] = torch.tensor(
                    action_to_save[0][0:2]
                )
        ...
        if success:
            ...
            torch.save(
                ...
                f"./collected_trajectories/tidy_house/successful_episode_{episode_id}_scene_{scene_id}_traj_num_{number_of_episodes}.pt",
            )
        #    input()
