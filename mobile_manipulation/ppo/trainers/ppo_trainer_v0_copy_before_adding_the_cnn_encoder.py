    ...
    def _eval_checkpoint(
     ...
     #   print(self.actor_critic.net.visual_encoder)
      ...
      #  input()
        ...
        if (Diffusion_policy and Save_data) or (Transformer_policy and Save_data) or (Flow_matching_policy and Save_data):
            print("cannot have both diffusion policy and data saving enabled at the same time")
            input()
        if (Diffusion_policy and Transformer_policy) or (Flow_matching_policy and Transformer_policy) or (Flow_matching_policy and Diffusion_policy):
        ...
        while number_of_episodes < num_eval_episodes:
       ...
       #     print("image shape == " , torch.from_numpy(robot_head_depth).unsqueeze(0).permute(0,3,1,2).to(device).shape)
          ...
          #  if number_of_steps==0:
           #     print("reset joint pos == " , np.array(initial_qpos))
            #    print("current q pos == " , robot_qpos)
             #   print("difference in joint pos == " , robot_qpos-np.array(initial_qpos))
              #  input()
    ...
    #
       ...
       #     print("info == " , info)
         ...
         #   obs, reward, done, info = env.step(step_action)
                ...
                if episode_success==1:
                    ...
                    if num_suc_episodes%50 == 49 and Save_data:
                        ...
                        if 'pick' in config.BASE_RUN_DIR :
                            ...
                            task_name='place'
                        elif 'nav' in config.BASE_RUN_DIR :
                            task_name='nav'
                        else:
            ...
            #    print("final q pos == " , env.env._env._sim.robot.arm_joint_pos)
             #   input()
                obs = env.reset()
                initial_qpos=env.env._env._sim.robot.arm_joint_pos.copy()
                current_step=0
