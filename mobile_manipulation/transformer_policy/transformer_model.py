...
class ActionInference(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.action_embeddings = nn.Sequential(
            nn.Linear(12, config.n_embd), nn.Tanh()   ## 12 is the number of action dimensions
        )
...
class SkillInference(nn.Module):
    ...
    def __init__(self, config):
        ...
        config.block_size = config.block_size * self.num_inputs  ## number of input tokens ( related to context length)
        ...
        ### config.num_states is [256 for the resnet output, and 19 or whatever for the concatenated fuse_keys obs]
