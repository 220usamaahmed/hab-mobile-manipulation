...
def read_dataset(
    ...
    while len(obss) < config.files_per_load:
        ...
        if os.path.exists(file):
            ...
            temp_actions[:, 7:10] = np.array([1,0,0])
            ...
            try:
                ...
                temp_actions[ii,  7:10] = np.array([0,1,0])
            except:
                pass
            ii = np.where(change > 0)[0]
            try:
                ...
                temp_actions[ii,  7:10] = np.array([0,0,1])
            except:
                pass
            ...
            if "abs_obj_start_sensor" in temp_obs[0].keys():
                for i in range(len(temp_obs)):
                    try:
                        temp_obs[i].pop("abs_obj_start_sensor")  ## .pop() removes an item from the list
           ...
           # print("single file actions shape == " , temp_actions.shape)
    ...
    #print("final total actions == " , actions.shape)
...
def producer(
    ...
    while True:
        if len(deque) < config.queue_size:
            s = time.perf_counter()
            obss, actions, done_idxs, rtg, timesteps=read_dataset(config, verbose, rng, context_length)
