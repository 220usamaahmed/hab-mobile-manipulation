...
class Diffusion_buffer(Dataset):
    ...
    def update_returns(self, score_model):
        ...
        for i in range(0, self.len):
            ...
            next_non_visual_states = self.next_non_visual_states[i].repeat(num_trajectories,1,1)
            next_visual_states = self.next_visual_states[i].repeat(num_trajectories,1,1)
            ...
            with torch.no_grad():
                q = score_model.calculateQ(next_visual_states, next_non_visual_states, torch.FloatTensor(fake_actions))
                print("Raw Q values of the generated actions shape == ", q.shape)
